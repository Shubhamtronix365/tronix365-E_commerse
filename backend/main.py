...
# =====================================================================
# SAVED ADDRESSES (ADDRESS BOOK) ENDPOINTS
# =====================================================================
...
@app.put("/addresses/{address_id}/set-default", response_model=AddressResponse)
...
# ==============================================================================
# TOWER ORDER & ON-DEMAND SOURCING ENDPOINTS (Steps 1 through 6)
# ==============================================================================

@app.post("/tower-orders", response_model=TowerOrderResponse, status_code=201)
...
@app.get("/tower-orders/user", response_model=List[TowerOrderResponse])
async def get_user_tower_orders(
    current_user: UserDB = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    """Retrieve all tower orders for the logged-in customer."""
    orders = (
...
@app.get("/tower-orders/{order_ref}", response_model=TowerOrderResponse)
async def get_tower_order_by_ref(
    order_ref: str,
    db: Session = Depends(get_db),
):
    """Retrieve details of a single tower order by order number or ID."""
    order = None
...
@app.post("/tower-orders/{order_id}/payment-proof", response_model=TowerOrderResponse)
async def submit_tower_order_payment(
    ...
    """
    Step 4: Customer will transfer money (NEFT / RTGS / IMPS)
    Customer records payment reference/UTR number and optional receipt screenshot.
    """
...
@app.get("/admin/tower-orders", response_model=List[TowerOrderResponse])
async def get_admin_tower_orders(
    ...
    """Admin retrieves all tower orders with optional status filter and text search."""
...
@app.put("/admin/tower-orders/{order_id}/status", response_model=TowerOrderResponse)
async def update_tower_order_status(
    ...
    """
    Step 2: Sales team will contact customer
    Admin/sales logs communication and updates workflow status.
    """
    order = db.query(TowerOrderDB).filter(TowerOrderDB.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Tower order not found")
...
@app.put("/admin/tower-orders/{order_id}/quotation", response_model=TowerOrderResponse)
async def update_tower_order_quotation(
    ...
    """
    Step 3: We will send P.I. / Quotation
    Admin generates or enters finalized unit price, total, and lead time breakdown.
    """
    order = db.query(TowerOrderDB).filter(TowerOrderDB.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Tower order not found")
...
@app.put("/admin/tower-orders/{order_id}/verify-payment", response_model=TowerOrderResponse)
async def verify_tower_order_payment(
    ...
    """
    Step 5: Once amount received
    Admin confirms NEFT/RTGS/IMPS funds received and marks status as 'in_production'.
    """
    order = db.query(TowerOrderDB).filter(TowerOrderDB.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Tower order not found")
    ...
    # Calculate estimated dispatch date based on factory lead days
...
@app.put("/admin/tower-orders/{order_id}/shipment", response_model=TowerOrderResponse)
async def update_tower_order_shipment(
    ...
    """
    Step 6: We will ship material (# Lead time: Factory + Shipping time)
    Admin records courier partner, tracking number, and live tracking link.
    """
    order = db.query(TowerOrderDB).filter(TowerOrderDB.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Tower order not found")
...
@app.get("/tower-products", response_model=List[Product])
async def get_tower_sourcing_products(
    db: Session = Depends(get_db),
):
    """Fetch all products marked as tower order only or eligible for sourcing."""
    return (
        db.query(ProductDB)
