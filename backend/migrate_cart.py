...
def migrate():
    with engine.connect() as conn:
        print("Starting cart_items migrations...")
        ...
        for col_name, col_type in columns_cart:
            try:
                conn.execute(
                    text(f"ALTER TABLE cart_items ADD COLUMN {col_name} {col_type}")
                )
                conn.commit()
                print(f"Added column '{col_name}' to 'cart_items' table.")
            except Exception as e:
                print(
                    f"Column '{col_name}' in 'cart_items' may already exist or error: {e}"
                )
                conn.rollback()
