...
def migrate():
    with engine.begin() as conn:
        ...
        try:
            conn.execute(text("ALTER TABLE cart_items ADD COLUMN bundle_id INTEGER REFERENCES bundles(id)"))
        ...
        try:
            conn.execute(text("ALTER TABLE cart_items ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP"))
